...
function open_intro_content() {
	$intro_size;
	switch(true) {
		case (is_page_template('template-l1.php') || is_page_template('template-sidebar.php')) :
			$intro_size = 9;
			break;
...
function intro_bg() {
	...
	$bg_img = (is_single() ? get_stylesheet_directory_uri() . '/images/single_post_intro.jpg' : (!empty($featured_img) && !is_search() ? $featured_img : '/wp-content/uploads/2017/05/default_featured_1.jpg'));
	...
	function single_post_intro() {
		...
		switch($post->post_type) {
			case 'post':
				...
				break;
			case 'event':
				$post_link = '/resources/upcoming-events/';
				break;
			case 'in_the_news':
				$post_link = '/resources/in-the-news/';
				break;
		}
		echo '<div class="post-breadcrumbs">
			<a href="' . $post_link . '"><img src="' . get_stylesheet_directory_uri() . '/images/button_left_blue.png" alt="Back" />&nbsp;&nbsp;&nbsp;Back</a>' . (!empty($cat[0]) ? '&nbsp;&nbsp;|&nbsp;&nbsp;<a href="' . get_category_link($cat[0]->term_id) . '">' . $cat[0]->name . '</a>' : "") . 
		'</div>
		<h1>' .
			$post->post_title . 
		'</h1>';
...
function the_entry_content() {
	if(is_archive() || is_home()) {
		the_excerpt('<button class="tiny">' . __( 'Read more...', 'jointswp' ) . '</button>');
	}
	else {
		the_content();
	}
